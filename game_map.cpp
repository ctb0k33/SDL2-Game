...
void GameMap::LoadMap( const char* name) {
	FILE* fp = NULL; //read file pointer
	fopen_s(&fp, name, "rb");
	if (fp == NULL) {
		return;
	}
	...
	for (int i = 0; i < MAX_MAP_Y; i++) {
		for (int j = 0; j < MAX_MAP_X; j++) {
			fscanf_s(fp, "%d", &game_map_.tile[i][j]);//read data
...
void GameMap::DrawMap(SDL_Renderer* screen) {
	...
	x2 = x1 + SCREEN_WIDTH + (x1 == 0 ? 0 : TILE_SIZE); // +x1=0 hoac tile_size

	map_y = game_map_.start_y_ / TILE_SIZE;// index cua o anh
